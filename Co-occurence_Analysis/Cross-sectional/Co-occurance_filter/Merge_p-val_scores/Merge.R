...
m_score=0
p_m=array(0,c(24,24,5)) #Change based on the number of microbes present

cluster="cluster2" #Input the cluster name for the results to be merged

count=1
methods=c("Bray-Curtis","GBLM","MI","Pearsons","Spearman")
w=c(1/6,1/2,1/6,1/12,1/12) #Weights for each method
for (i in methods){
...
p_f[p_f==0]<-NA
#p-values with NA become 0 because if NA is present it is assigned 0 by the simes.test function
